...
descriptions:
    ...
    UsersTweets.py:
        ...
        this metod filters and clean Tweets. this cleaning proccess contains removing stop words from Tweet's text, vectorizing each text's word, POS tagging text(using nltk), and finally calculating it's topic by averaging vectors.(this method is not complete yet.)
    ...
    TweetURLIDF:
        this class proccesses cleaned data we created from UserTweets.py class, it calculates IDF of each url. proccess is exactly like above except that here we dont want to do proccess on the URL not the text.
        first it creates object from each user and their tweet to make the proccess easy, then compares each URLs with each other to calculate their similarity. this similarity is being used when calculates the IDF ot Tweets.it stores proccessed data in a json file.(this method is also not complete yet).

        Execution:
            run `./TweetURLIDF.py` in your terminal to execute the software. a 'user_tweets.json' file is the result.

        Note: this proccess needs to be compitable with what we created on the last part.
        if you call for an attribute which you didn't attach to cleaned jon file, it gives an error.
